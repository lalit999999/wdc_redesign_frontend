...
export function AdminAnnouncements() {
  ...
  const handleCreate = () => {
    ...
    addAnnouncement({ title, message, date: dateStr, time: timeStr, type, important });
    setTitle('');
    setMessage('');
    setType('info');
    setImportant(false);
      ...
      <main className="flex-1 lg:ml-60 pt-14 lg:pt-0">
        <div className="max-w-6xl mx-auto p-4 sm:p-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <h1 className="text-3xl font-semibold text-foreground">Manage Announcements</h1>
            <button
          ...
          {showForm && (
            <div className="mb-8 p-6 bg-card border border-border rounded-lg">
              <h2 className="text-lg font-semibold mb-4 text-foreground">New Announcement</h2>
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm mb-2 text-foreground">Message</label>
                  <textarea
                    value={message}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm mb-2 text-foreground">Type</label>
                    <select
                      value={type}
          ...
          {announcements.length === 0 && (
            <div className="p-12 bg-card border border-border rounded-lg text-center">
              <p className="text-muted-foreground">No announcements yet. Click "Create Announcement" to get started.</p>
            </div>
          )}
        </div>
      </main>
